...
namespace Popeye.Modules.Enemies.Components
{
    ...
    public class ParabolicProjectile : RecyclableObject
    {
        ...
        private void Update()
        {
            if (_playerTransform != null)
            {
                ...
                if (_shoot)
                {
                    ...
                    float time;
                    RaycastHit hit;
                    if (Physics.Raycast(predictPos + Vector3.up, Vector3.down, out hit, 10f,
                            _defaultProbingConfig.CollisionLayerMask, _defaultProbingConfig.QueryTriggerInteraction))
                    {
        ...
        public void ShootRandom()
        {
            ...
            RaycastHit hit;
            if (Physics.Raycast(_randomTarget + Vector3.up, Vector3.down, out hit, 10f,
                    _defaultProbingConfig.CollisionLayerMask, _defaultProbingConfig.QueryTriggerInteraction))
            {
                var startRot = Quaternion.LookRotation(hit.normal) * Quaternion.Euler(new Vector3(0, 90, 90f));
                _particleFactory.Create(_attackPreview, hit.point, startRot);
